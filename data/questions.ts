import type { Question, ComplianceReport } from '../types';
...
// A portion of the final report, generated by a rule.
type ReportResult = Omit<ComplianceReport, 'timestamp' | 'version' | 'system_id' | 'owner' | 'notes' | 'hash'>;
...
const complianceRules: ComplianceRule[] = [
  // Rule 1: Out of Scope
  {
    condition: (answers) => answers['scope_location'] === 'outside_eu',
    result: {
  ...
  // Rule 2: Prohibited Systems
  {
    condition: (answers) => {
      ...
      return ['social_scoring', 'manipulative', 'exploitative'].includes(prohibitedAnswer);
    },
    result: {
  ...
  // Rule 3: High-Risk Systems
  {
    condition: (answers) => {
      ...
      return ['biometrics', 'critical_infra', 'essential_services'].includes(highRiskPurpose);
    },
    result: {
  ...
  {
    condition: (answers) => answers['scope_location'] === 'eu_market',
    result: {
...
const defaultResult: ReportResult = {
...
export const evaluateCompliance = (answers: Record<string, string>): ComplianceReport => {
  ...
  for (const rule of complianceRules) {
      if (rule.condition(answers)) {
          return { ...reportBase, ...rule.result };
      }
  }

  return { ...reportBase, ...defaultResult };
};