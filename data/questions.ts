...
import type { Question, ComplianceReport, Classification } from '../types';
...
export const evaluateCompliance = (answers: Record<string, string>): ComplianceReport => {
  ...
  if (prohibitedAnswer === 'social_scoring' || prohibitedAnswer === 'manipulative' || prohibitedAnswer === 'exploitative') {
    ...
    obligations = ['Cease development and deployment in the EU.'];
    references = ['Article 5'];
    return { ...reportBase, classification, summary, obligations, references };
  }
  ...
  if (highRiskPurpose === 'biometrics' || highRiskPurpose === 'critical_infra' || highRiskPurpose === 'essential_services') {
    ...
    obligations = [
        ...
        'Ensure transparency and provide instructions for use (Article 13)',
  ...
  if (answers['scope_location'] === 'eu_market') {
     ...
     obligations = ['Ensure users are aware they are interacting with an AI system (Article 52).', 'Consider adopting codes of conduct for non-high-risk AI.'];
  ...
  return {
    ...
    obligations: ['Voluntary adoption of codes of conduct.'],
